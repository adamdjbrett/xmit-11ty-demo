# xmit.000000076.xyz
xmit xmit.000000076.xyz _site
npm run deploy

## Compression reproduce
 ```node
 npm run build            # Generates minified + inlined site
 ```
```
node -e 'const {execSync}=require("node:child_process");const zlib=require("node:zlib");const fs=require("node:fs");function c(p){const d=fs.readFileSync(p);fs.writeFileSync(p+".gz",zlib.gzipSync(d,{level:9}));fs.writeFileSync(p+".br",zlib.brotliCompressSync(d,{params:{[zlib.constants.BROTLI_PARAM_QUALITY]:11}}));}function w(d){for(const f of fs.readdirSync(d)){const full=d+"/"+f;const st=fs.statSync(full);if(st.isDirectory())w(full);else if(/\\.(html|css|js)$/.test(f))c(full);}}w("_site");console.log("Compression complete");'
```

```
npm run deploy           # Copies xmit.toml (if present) and deploys with XMIT
```
